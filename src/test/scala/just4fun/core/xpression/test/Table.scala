...
import just4fun.core.xpression.{BuildContext, PropXNode, XNode}

trait Table[S <: Table[S]] extends Schema[S] with Loggable {this: S =>
	type OBJ = TabObj
	...
	class Column[T: Manifest](val index: Byte, val name: String, val valueType: PropType[T] with BaseType[_]) extends Prop[T] with PropXNode {
		nodeType = {
		...
		override def build(cxt: BuildContext, builder: StringBuilder, parent: Option[XNode]): Unit = {
			builder ++= (if (cxt.indexBasedName) "x" + index else name)
		}
	}
}
