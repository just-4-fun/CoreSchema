...
trait Schema[S <: Schema[S]] {
	...
	val theVersion = 0
	// Prefix char of index-based prop name
	val propPrefix: Char = 'x'
	...
	/* PROPS MANAGEMENT */
	protected def newProp[T: Manifest](index: Byte, name: String, valueType: PropType[T] with BaseType[_]): PROP[T]

	protected def constructProp[T: Manifest](index: Byte, name: String, valueType: PropType[T] with BaseType[_], initCode: S#PROP[_] => Unit = null): S#PROP[T] = {
		val p = newProp(index, name, valueType).asInstanceOf[S#PROP[T]]
	...
	protected def addProp[T: Manifest](p: S#PROP[T]) = tmpProps.find(_.index == p.index) match {
		case None => tmpProps = p :: tmpProps
			//			logd(s"Prop with index ${p.index } is defined ok.")
			if (p.index >= propsSize) propsSize = (p.index + 1).toByte
		case _ => loge(msg = s"Prop with index ${p.index} is defined. Indexes should be unique.")
	}
	...
	trait Obj {
		...
		/** NON-TYPE-SAFE value assignment. NEITHER register Prop change NOR update field */
		...
		def clearChanged() = propsChanged.clear()
	...
	abstract class Prop[T] {
		...
		var forceName = false
		lazy val indexName: String = if (forceName) name else s"$propPrefix$index"
		...
		def info: String = s"[$indexName,$name,$typeName]"
		...
		def linkField(f: Field): Boolean = if (f.getName != name) false
			...
			loge(msg = s"Can't link prop $name with same-named field due to the type mismatch. Prop type= $typeName; field type= ${f.getType}")
			false
		}
