...
case class BuildContext(sqlContext: Int = SqlContext.CODE, indexBasedName: Boolean = true)


object XNode {
	implicit class NumConst[T: Numeric](val value: T) extends ConstXNode with NumXNode
	implicit class StrConst(val value: String) extends ConstXNode with StrXNode
...
trait XNode {
	...
	def ===(node: XNode): BoolXBranch = node match {
		case null => BoolXBranch(this, NullXNode, XOp.IS)
		case _ => BoolXBranch(this, node, XOp.===)
	}
	def ==!(node: XNode) = node match {
		case null => BoolXBranch(this, NullXNode, XOp.ISNOT)
		case _ => BoolXBranch(this, node, XOp.==!)
	}
...
object NullXNode extends XNode {
...
trait XBranch extends XNode {
	...
	if (!isInstanceOf[LogicXBranch]) {
		...
		//		logi(s"Ltp= ${n1.nodeType};  Rtp= ${n2.nodeType}")
	}
	//	logi(s"new Branch ? ${group} [$op]:  n1=${n1.toExpr()}  ? ${n1.group};  n2=${n2.toExpr()}   ? ${n2.group}")
	//	logi(toExpression())
...
trait PropXNode extends NumXNode with StrXNode with BoolXNode
